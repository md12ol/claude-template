...
git -C "$DOCS" check-ignore -q "$WORK_PARKED" && echo "not tracked — skipping" || {
    git -C "$DOCS" add -A "$WORK_CURRENT" "$WORK_PARKED"
    git -C "$DOCS" commit -m "park: <slug> — blocked on <the event>"
    git -C "$DOCS" push
}
```
