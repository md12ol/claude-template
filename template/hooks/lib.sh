...
# used: on the fork layout .claude/ is its own repository, so running from inside it reports
# .claude as the top level and every path below would gain an extra .claude/.
claude_paths() {
