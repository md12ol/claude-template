...
**This is not `/done`**, which is for finished work: everything `/park` moves comes back via
`/load <slug>`.
...
days, and `handoff.md` is written for a reader who has forgotten everything.

## 3. Stamp `handoff.md` with what would unblock it

Write **one** line directly under the handoff's heading:
`**Blocked on:** <the concrete event that would make this workable again>`. One line, because the
brief prints that line and nothing else beside the slug; context belongs in the handoff's body.

**Name an event, not a feeling.** "Waiting on review" is not a blocker; "PR #482 merging" is. Could
someone else read the line and tell you the moment it came true? If nothing concrete would unblock
it, the task is deprioritized rather than blocked: `/done` with an honest outcome,
`work/deferred.md`, or a tracker issue where `TRACKER_FIRST="yes"` and that file is gone.

The rest of the stamping is not yours: `task.sh park` rewrites the `Machine:` line's `saved` to
`parked` and appends `Resume with /load <slug>.` after your blocker line. With no blocker line it
warns, still parks, and the brief then shows "no blocker recorded".
...
- **Never park without saving**, and **never park a finished task**: whoever finds it reads it as
  unfinished. Finished work is `/done`.
