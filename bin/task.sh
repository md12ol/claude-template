...
#
#     paths                    OWNER_DIR/WORK_*/PEOPLE/MACHINES, eval-able
#     pull                     fast-forward the docs clone, never the code repo
#     start "<objective>"      create the live task dir and seed history.md
...
#     audit [file…]            union-merge collision and structure audit
#     commit "<message>"       commit the task directories, and push if there is an origin
#
...
commit)
    ...
    for d in "$WORK_CURRENT" "$WORK_PARKED" work/archive; do
        [[ -n "$d" && -e "$d" ]] && git add -A "$d" 2>/dev/null
    done
    if git diff --cached --quiet 2>/dev/null; then
        echo "task.sh commit: nothing staged in the task directories"
        exit 0
    fi
